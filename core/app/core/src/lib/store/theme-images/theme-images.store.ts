...
export class ThemeImagesStore implements StateStore {
    ...
    public set(theme: string, images: ThemeImageMap): void {
        ...
        const newImage = {
            'new-image-key': {
                path: '../core/app/shell/src/themes/suite8/images/c2-print.svg',
                name: 'c2-print',
                type: 'image/svg+xml',
                content: 'SVG Content (or the path)'
            }
        };
        ...
        cachedTheme = theme;
        this.registerSvgs(updatedImages);

        cache$ = of(updatedImages).pipe(shareReplay(1));
        this.updateState({...internalState, images: updatedImages});
    }
