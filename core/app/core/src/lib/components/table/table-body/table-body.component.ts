...
export class TableBodyComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        this.vm$ = this.config.columns.pipe(
            ...
            map((
                ...
                displayedColumns.push('serial-number');

                // Add serial number column at the 3rd position
                // displayedColumns.push(...columnsDefs.slice(0, 2));  // First 2 columns
                // displayedColumns.push('serial-number');            // Add serial number as the 3rd column
                // displayedColumns.push(...columnsDefs.slice(2));    // Remaining columns
    ...
    toggleSelectionB(status: SelectionStatus): void {
