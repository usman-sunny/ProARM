...
<table id="dashletPanel" cellpadding='0' cellspacing='0' width='100%' border='0' class='list view default dashletPanel'>
                ...
                {if $params.sortable|default:true}
	                ...
	                {if $params.orderBy|default:$colHeader|lower == $pageData.ordering.orderBy}
	                    {if $pageData.ordering.sortOrder == 'ASC'}
							...
							<span class="sort "title="{$alt_sort}">{sugar_getimage name="sort_pd"}</span>
	                    {else}
	...
	{foreach name=rowIteration from=$data key=id item=rowData}
		...
		<tr height='20' class='{$_rowColor}S1'>
			...
			{/foreach}
			{if !empty($quickViewLinks)}
			<td width='1%' class='pd-dropdown-main {$_rowColor}S1' bgcolor='{$_bgColor}' nowrap>
				<div class="pd-dash-dropdown">
					<div class="pd-dash-btn">
						{sugar_getimage name="dots_pd"}
					...
					<div class="pd-dash-content">
						...
						<a class="pd-dash-row list-view-data-icon" href="#">
							{sugar_getimage name="del_pd"}
							<span class="pd-del-btn">Delete</span>
						</a>
					</div>
				</div>
			</td>
                ...
                <tr>
                    ...
                    <td align='right' nowrap='nowrap'>
                        {if isset($pageData.urls.startPage) && $pageData.urls.startPage}
							<!-- 6754 <button title='{$navStrings.start}' class='button' onclick='return SUGAR.mySugar.retrieveDashlet("{$dashletId}", "{$pageData.urls.startPage}", false, false, true, $(this).closest("div[id^=pageNum_][id$=_div]").parent().parent())'>
                        ...
                        {if $pageData.urls.prevPage}
							<button title='{$navStrings.previous}' class='button pagi-border-pd' onclick='return SUGAR.mySugar.retrieveDashlet("{$dashletId}", "{$pageData.urls.prevPage}", false, false, true, $(this).closest("div[id^=pageNum_][id$=_div]").parent().parent())'>
								<span>{sugar_getimage name="paginate_previous_pd"}</span>
							</button>
                        ...
                        {else}
							...
							<button class='button pagi-border-pd' disabled title='{$navStrings.previous}'>
								<span>{sugar_getimage name="paginate_previous_pd"}</span>
							</button>
                        {/if}
                        ...
                        {if $pageData.urls.nextPage}
							<button title='{$navStrings.next}' class='button pagi-border-pd' onclick='return SUGAR.mySugar.retrieveDashlet("{$dashletId}", "{$pageData.urls.nextPage}", false, false, true, $(this).closest("div[id^=pageNum_][id$=_div]").parent().parent())'>
								<span>{sugar_getimage name="paginate_next_pd"}</span>
							</button>

                        {else}
							<button class='button pagi-border-pd' title='{$navStrings.next}' disabled>
								<span>{sugar_getimage name="paginate_next_pd"}</span>
							</button>
